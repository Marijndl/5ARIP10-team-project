...
for part in range(partitions):
    ...
    start_idx = int(part * (lengthhh / partitions))
    end_idx = int(start_idx + (lengthhh / partitions))

    for idx in range(start_idx, end_idx):

        print(f"{idx}/{lengthhh}")
        data_2d = np.genfromtxt(os.path.join(directory_2D, file_names_2D[idx]), delimiter=",")
        data_3d = np.genfromtxt(os.path.join(directory_3D, file_names_3D[math.floor(idx/10)]), delimiter=",")
