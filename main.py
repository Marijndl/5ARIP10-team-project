...
def train_model(model, criterion, optimizer, train_loader, num_epochs=186):
    for epoch in range(num_epochs):
        ...
        for batch_idx, input in loop:
            # Get data to cuda
            # origin_3D = input['origin_3D'].to(device)
            # shape_3D = input['shape_3D'].to(device)
            # origin_2D = input['origin_2D'].to(device)
            # shape_2D = input['shape_2D'].to(device)
            # offset = input['offset'].to(device)
